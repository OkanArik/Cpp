...
public:
	...
	void passFailStatus(Student* studentArray, int studentsCount);
	void printStudentInformation(Student* studentArray, int studentsCount);
};

void Teacher::passFailStatus(Student *studentArray ,int studentsCount) {
	float average = 0.0;
	for (int i = 0; i < studentsCount; i++) {
		average =average+ studentArray[i].getStudentGrade();
	}
	average = (float)average / studentsCount;
	cout << "Class average : " << average <<studentsCount<< endl;
	cout << "------------------" << endl;
	for (int i = 0; i < studentsCount; i++) {
...
void Teacher::printStudentInformation(Student *studentArray, int studentsCount) {
	for (int i = 0; i < studentsCount; i++) {
