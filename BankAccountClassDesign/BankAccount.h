...
bankAccount::bankAccount(const bankAccount& othAcoount) {
	personName = othAcoount.personName;
	personSurname = othAcoount.personSurname;
	personPhone = othAcoount.personPhone;
	accountBalance = othAcoount.accountBalance;
...
void bankAccount::sendMoney(bankAccount& othAccount, float amount) {
	while (amount <= 0 || amount > this->accountBalance) {
		cout << "Invalid amount ; please enter valid amount : ";
		cin >> amount;
	}
